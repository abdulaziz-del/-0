...
# ════════════════════════════════════════════════════════
# ── التدابير غير الجمركية — UNCTAD TRAINS / WITS API ──
# ════════════════════════════════════════════════════════

_ntm_cache = {}   # key: reporter_iso → list
...
NTM_TYPE_MAP = {
    ...
    "C": ("PS",  "Pre-shipment inspection and formalities",        "TBT Agreement"),
    "D": ("CPS", "Contingent trade-protective measures",           "Anti-Dumping Agreement"),
    "E": ("QR",  "Non-automatic licensing / prohibitions",         "GATT 1994 Art. XI"),
    "F": ("PC",  "Price-control measures",                         "GATT 1994 Art. VI"),
    "G": ("FIN", "Finance measures",                               "GATT 1994"),
    "H": ("COM", "Competition measures",                           "GATT 1994"),
...
SAUDI_HS = {"27","28","29","31","39","40","72","76","08","15","17","48","70","68"}
GULF_ISO = {"SAU","ARE","KWT","QAT","BHR","OMN"}

def _ntm_saudi_impact(rep, par, hs):
    if par and par.upper() in GULF_ISO:
        return True
    if hs and hs[:2] in SAUDI_HS:
        return True
    return False

def _ntm_risk(code, ntype, impact):
    if code and code[0].upper() in {"D","L"} and impact:
        return "high"
    if code and code[0].upper() == "E" and impact:
        return "high"
    if ntype in ("SPS","TBT") and impact:
        return "medium"
    if code and code[0].upper() in {"D","E","L"}:
        return "medium"
    return "low"

def _ntm_action(risk, ntype, code):
    if risk == "high":
        return "dispute" if (code and code[0].upper() == "D") else "stc"
    if risk == "medium":
        return "consult" if ntype in ("SPS","TBT") else "notify"
    return "monitor"

def _parse_wits_row(row, reporter_iso=""):
    code    = (row.get("ntmCode") or row.get("NTMCode") or "").strip()
    prefix  = code[0].upper() if code else ""
    tinfo   = NTM_TYPE_MAP.get(prefix, ("OTH", "Other measures", "GATT 1994"))
    rep     = (row.get("reporterISO3") or row.get("reporterIso") or reporter_iso or "").upper()
    par     = (row.get("partnerISO3")  or row.get("partnerIso")  or "WLD").upper()
    hs      = row.get("hsCode") or row.get("productCode") or ""
    impact  = _ntm_saudi_impact(rep, par, hs)
    risk    = _ntm_risk(code, tinfo[0], impact)
    action  = _ntm_action(risk, tinfo[0], code)
    return {
        "id":                  rep + "_" + code + "_" + hs,
        "reporter_iso":        rep,
        "reporter_country":    row.get("reporterName") or row.get("reporter") or rep,
        "partner_iso":         par,
        "partner_country":     row.get("partnerName")  or row.get("partner")  or par,
        "hs_code":             hs,
        ...
        "ntm_code":            code,
        "ntm_type":            tinfo[0],
        "ntm_description":     tinfo[1],
        "measure_summary_en":  row.get("measureDescription") or row.get("ntmDescription") or tinfo[1],
        "measure_summary_ar":  "",
        "legal_classification": tinfo[2],
        "wto_agreement":       tinfo[2],
        "wto_agreements_all":  [tinfo[2]],
...
def _fetch_wits(reporter, partner="all", hs="all", year="all", ntm_type="all"):
    """جلب من WITS TRAINS API الرسمي"""
    base = "https://wits.worldbank.org/API/V1"
    url  = "{}/wits/datasource/TRN/dataavailability/{}/{}/{}/{}/{}".format(
        base, reporter.upper(),
        partner or "all", hs or "all",
        str(year) if year else "all",
        ntm_type or "all"
    )
    hdrs = {"Accept": "application/json", "User-Agent": "WTO-Monitor/1.0"}
    try:
        r = requests.get(url, headers=hdrs, params={"format": "json"}, timeout=30)
        log.info("WITS %s → %d", url[:80], r.status_code)
        if r.status_code != 200:
            return [], r.status_code
        d = r.json()
        rows = (
            d.get("wits:ntmDataResult", {}).get("wits:ntmData", [])
            or d.get("ntmData", [])
            or d.get("data", [])
        ...
        if isinstance(rows, dict):
            rows = [rows]
        return [_parse_wits_row(m, reporter) for m in rows if isinstance(m, dict)], 200
    except Exception as e:
        log.error("WITS fetch error: %s", e)
        return [], 0

def _translate_ntm_batch(measures):
    """ترجمة ملخصات NTM للعربية دفعة واحدة"""
    if not CLAUDE_KEY:
        return measures
    need = [m for m in measures if not m.get("measure_summary_ar") and m.get("measure_summary_en")][:12]
    if not need:
        return measures
    texts    = [m["measure_summary_en"] for m in need]
    numbered = "\n".join([str(i+1) + ". " + t for i, t in enumerate(texts)])
    prompt   = "ترجم أوصاف التدابير التجارية التالية للعربية الفصحى القانونية. اكتب الرقم ثم الترجمة فقط:\n" + numbered
    try:
        r = requests.post(
            "https://api.anthropic.com/v1/messages",
            headers={"x-api-key": CLAUDE_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json"},
...
@app.route("/api/ntm/search", methods=["GET"])
def ntm_search():
    """
    جلب التدابير غير الجمركية من WITS TRAINS API
    params: reporter_iso, partner_iso, hs_code, year, ntm_type,
            risk_level, impact_saudi, page, page_size, refresh
    """
    reporter  = request.args.get("reporter_iso", "all").upper()
    partner   = request.args.get("partner_iso", "all")
    hs        = request.args.get("hs_code", "all")
    year      = request.args.get("year", "all")
    ntm_type  = request.args.get("ntm_type", "all").upper()
    risk_f    = request.args.get("risk_level", "")
    saudi_f   = request.args.get("impact_saudi", "")
    pg        = max(1, int(request.args.get("page", 1)))
    pg_size   = min(100, int(request.args.get("page_size", 50)))
    force     = request.args.get("refresh", "0") == "1"
    translate = request.args.get("translate", "1") == "1"

    cache_key = reporter + "_" + partner + "_" + hs + "_" + year + "_" + ntm_type

    # فحص الكاش
    with _ntm_lock:
        cached = _ntm_cache.get(cache_key)

    if not force and cached:
        all_data = cached
    else:
        # جلب من WITS
        all_data, status = _fetch_wits(reporter, partner, hs, year, ntm_type)
        if not all_data and reporter == "ALL":
            # fallback: جلب لكبار الدول
            for iso in ["USA", "EU", "CHN", "IND", "JPN", "GBR", "DEU", "FRA", "AUS", "CAN", "BRA", "KOR"]:
                rows, _ = _fetch_wits(iso, partner, hs, year, ntm_type)
                all_data.extend(rows)
                if len(all_data) >= 200:
                    break
        if translate and all_data:
            all_data = _translate_ntm_batch(all_data)
        with _ntm_lock:
            _ntm_cache[cache_key] = all_data
        log.info("NTM cached %d measures for key=%s", len(all_data), cache_key)

    # فلترة
    filtered = all_data
    if risk_f:
        filtered = [m for m in filtered if m.get("risk_level") == risk_f]
    if saudi_f == "1":
        filtered = [m for m in filtered if m.get("impact_saudi_exports")]

    total     = len(filtered)
    page_data = filtered[(pg-1)*pg_size : pg*pg_size]
    ...
    return jsonify({
        ...
        "stats": {
            "total":   total,
            "high":    sum(1 for m in filtered if m.get("risk_level") == "high"),
            "medium":  sum(1 for m in filtered if m.get("risk_level") == "medium"),
            "low":     sum(1 for m in filtered if m.get("risk_level") == "low"),
            "saudi":   sum(1 for m in filtered if m.get("impact_saudi_exports")),
            "dispute": sum(1 for m in filtered if m.get("recommended_action") == "dispute"),
        }
    })
...
def ntm_analyze():
    """تحليل قانوني كامل لتدبير NTM واحد عبر Claude"""
    if not CLAUDE_KEY:
        return jsonify({"error": "No Claude key", "analysis": ""})
    try:
        m        = request.get_json() or {}
        ntype    = m.get("ntm_type", "")
        ...
        year     = str(m.get("year", ""))
        desc_en  = m.get("measure_summary_en") or m.get("ntm_description") or ""
        legal    = m.get("legal_text_reference", "")
        wto_agr  = m.get("wto_agreement", "")

        wto_ctx  = {
            "SPS": "اتفاقية SPS — المادة 5 (تقييم المخاطر) ومعايير Codex/OIE/IPPC",
            "TBT": "اتفاقية TBT — المادة 2 (عدم التمييز) ومعايير ISO/IEC",
            "QR":  "GATT 1994 المادة XI — الإلغاء العام للقيود الكمية",
            "CPS": "اتفاقية مكافحة الإغراق (Anti-Dumping Agreement) المادة 17",
            "SBS": "اتفاقية الدعم والتدابير التعويضية (SCM) المادة 25",
        }.get(ntype, "GATT 1994 + " + (wto_agr or "الاتفاقية ذات الصلة"))

        prompt = "\n".join([
            "أنت مستشار قانوني متخصص في اتفاقيات منظمة التجارة العالمية (WTO) والتدابير غير الجمركية (NTMs).",
            "حلّل التدبير التالي بالعربية الفصحى القانونية الرسمية:",
            "",
            "الدولة المطبِّقة: " + reporter,
            "الدولة الشريكة: "  + partner,
            "كود HS: "          + hs_code + " | المنتج: " + product,
            "رمز التدبير: "     + ntm_code + " | النوع: " + ntype + " — " + desc_en,
            "المرجع القانوني: " + legal,
            "السنة: "           + year,
            "",
            "الإطار القانوني: " + wto_ctx,
            "",
            "=== الملخص التنفيذي ===",
            "4-5 جمل عن جوهر التدبير وأثره على صادرات المملكة (النفط/البتروكيماويات/الألومنيوم/التمور/SABIC)",
            "",
            "=== التحليل القانوني ===",
            "1. الأساس القانوني وفق " + wto_ctx,
            "2. هل يتوافق مع الاستثناءات المشروعة في GATT Art. XX؟",
            "3. الأثر التجاري على صادرات المملكة تحديداً",
            "4. حقوق الطعن وآليات DSU المتاحة",
            "",
            "=== مبررات التوصية ===",
            "حدد توصية واحدة: (متابعة / إخطار / استشارة / STC / نزاع DSU) مع مبرراتها القانونية.",
            "",
        ...
        if r.status_code == 200:
            ...
            ratio_m = re.search(r"مبررات التوصية\s*={0,3}\s*([\s\S]*)", text, re.I)
            return jsonify({
                "analysis":         text,
                "executive_summary": exec_m.group(1).strip()  if exec_m  else text[:500],
                "action_rationale":  ratio_m.group(1).strip() if ratio_m else text[-400:],
            })
...
@app.route("/api/ntm/stats", methods=["GET"])
def ntm_stats():
    """إحصائيات كاش NTM"""
    all_data = []
    with _ntm_lock:
    ...
    return jsonify({
        ...
        "cached_keys":    list(_ntm_cache.keys()),
    })
