...
# ════════════════════════════════════════════════
# ── التدابير غير الجمركية (NTM / WITS TRAINS) ──
# ════════════════════════════════════════════════

# كاش NTM في الذاكرة (مفصول عن كاش ePing)
_ntm_cache  = {}   # key: "reporter_iso" → list of records
_ntm_lock   = threading.Lock()
_ntm_log    = []   # سجل عمليات الجلب

WITS_BASE   = "https://wits.worldbank.org/API/V1"

# خريطة رموز NTM → نوع + وصف + اتفاقية WTO
NTM_TYPE_MAP = {
    "A": ("SPS",  "Sanitary and Phytosanitary measures",          "SPS Agreement"),
    "B": ("TBT",  "Technical Barriers to Trade",                  "TBT Agreement"),
    "C": ("PS",   "Pre-shipment inspection and other formalities","TBT Agreement"),
    "D": ("CPS",  "Contingent trade-protective measures",         "Anti-Dumping Agreement"),
    "E": ("QR",   "Non-automatic import licensing / prohibitions","GATT 1994 Art. XI"),
    "F": ("PS2",  "Price-control measures",                       "GATT 1994 Art. VI"),
    "G": ("FIN",  "Finance measures",                             "GATT 1994"),
    "H": ("COM",  "Measures affecting competition",               "GATT 1994"),
    "I": ("TRI",  "Trade-related investment measures",            "TRIMS Agreement"),
    "J": ("DST",  "Distribution restrictions",                    "GATS"),
    "K": ("ROS",  "Restrictions on post-sales services",          "GATS"),
    "L": ("SBS",  "Subsidies",                                    "SCM Agreement"),
    "M": ("GPA",  "Government procurement restrictions",          "GPA"),
    "N": ("IP",   "Intellectual property",                        "TRIPS Agreement"),
    "O": ("ROR",  "Rules of origin",                              "GATT 1994 Art. IX"),
    "P": ("EXP",  "Export-related measures",                      "GATT 1994 Art. XI"),
}

# المنتجات الرئيسية السعودية لتحديد الأثر تلقائياً
SAUDI_HS_SECTORS = {
    "27": "بترول وطاقة",
    "29": "بتروكيماويات",
    "28": "كيماويات",
...
SAUDI_EXPORTERS  = ["SAU", "ARE", "KWT", "QAT", "BHR", "OMN"]   # دول الخليج كمصدرين


def _detect_saudi_impact(reporter_iso: str, partner_iso: str, hs_code: str) -> bool:
    """تحديد ما إذا كان التدبير يؤثر على صادرات المملكة."""
    if partner_iso and partner_iso.upper() in SAUDI_EXPORTERS:
        return True
    if reporter_iso and reporter_iso.upper() in SAUDI_EXPORTERS:
        return False   # المملكة هي المطبِّقة
    if hs_code:
        chapter = hs_code[:2]
        if chapter in SAUDI_HS_SECTORS:
            return True
    return False


def _classify_risk(ntm_code: str, ntm_type: str, impact_saudi: bool) -> str:
    """تصنيف مستوى الخطر بناءً على نوع التدبير والأثر."""
    high_codes = {"D", "E", "L"}   # CPS / QR / Subsidies
    if ntm_code and ntm_code[0].upper() in high_codes and impact_saudi:
        return "high"
    if ntm_type in ("SPS", "TBT") and impact_saudi:
        return "medium"
    if ntm_code and ntm_code[0].upper() in high_codes:
        return "medium"
    return "low"


def _recommend_action(risk: str, ntm_type: str, ntm_code: str) -> str:
    """اختيار التوصية المناسبة."""
    if risk == "high":
        if ntm_code and ntm_code[0].upper() == "D":
            return "dispute"
        return "stc"
    if risk == "medium":
        if ntm_type in ("SPS", "TBT"):
            return "consult"
        return "notify"
    return "monitor"


def _parse_wits_row(row: dict, reporter_iso: str = "") -> dict:
    """تحويل سجل WITS الخام إلى نموذج موحّد."""
    ntm_code   = (row.get("ntmCode") or row.get("NTMCode") or "").strip()
    prefix     = ntm_code[0].upper() if ntm_code else ""
    type_info  = NTM_TYPE_MAP.get(prefix, ("OTH", "Other measures", "GATT 1994"))
    rep_iso    = (row.get("reporterISO3") or row.get("reporterIso") or reporter_iso or "").upper()
    par_iso    = (row.get("partnerISO3")  or row.get("partnerIso")  or "WLD").upper()
    hs_code    = row.get("hsCode") or row.get("productCode") or ""
    impact     = _detect_saudi_impact(rep_iso, par_iso, hs_code)
    risk       = _classify_risk(ntm_code, type_info[0], impact)
    action     = _recommend_action(risk, type_info[0], ntm_code)

    return {
        "id":                    rep_iso + "_" + ntm_code + "_" + hs_code,
        "reporter_iso":          rep_iso,
        "reporter_country":      row.get("reporterName") or row.get("reporter") or rep_iso,
        "partner_iso":           par_iso,
        "partner_country":       row.get("partnerName") or row.get("partner") or par_iso,
        "hs_code":               hs_code,
        "hs_version":            row.get("hsVersion") or "HS2017",
        "product_description":   row.get("productDescription") or row.get("product") or "",
        "year":                  int(row["year"]) if row.get("year") else None,
        "ntm_code":              ntm_code,
        "ntm_type":              type_info[0],
        "ntm_description":       type_info[1],
        "measure_summary_en":    row.get("measureDescription") or row.get("ntmDescription") or type_info[1],
        "measure_summary_ar":    "",
        "legal_classification":  type_info[2],
        "wto_agreement":         type_info[2],
        "wto_agreements_all":    [type_info[2]],
        "legal_text_reference":  row.get("legalReference") or row.get("legalTextReference") or "",
        "affected_products":     row.get("affectedProducts") or "",
        "start_date":            row.get("startDate") or "",
        "end_date":              row.get("endDate") or "",
        "impact_saudi_exports":  impact,
        "risk_level":            risk,
...
def _fetch_wits_ntm(reporter_iso: str, partner_iso: str = "all",
                    hs_code: str = "all", year: str = "all",
                    ntm_type: str = "all") -> list:
    """جلب التدابير من WITS API — يعيد قائمة مُعالجة."""
    url = (
        WITS_BASE + "/wits/datasource/TRN/dataavailability"
        + "/" + reporter_iso.upper()
        + "/" + (partner_iso or "all")
        + "/" + (hs_code or "all")
    ...
    headers = {"Accept": "application/json", "User-Agent": "WTO-Monitor/1.0"}
    try:
        r = requests.get(url, headers=headers, params={"format": "json"}, timeout=30)
        log.info("WITS fetch %s → %d", url[:80], r.status_code)
        if r.status_code != 200:
            return []
        data = r.json()
        # WITS يغلّف البيانات في مسارات متعددة
        measures = (
            data.get("wits:ntmDataResult", {}).get("wits:ntmData", [])
            or data.get("ntmData", [])
            or data.get("data", [])
        ...
        if isinstance(measures, dict):
            measures = [measures]
        return [_parse_wits_row(m, reporter_iso) for m in measures if isinstance(m, dict)]
    except Exception as e:
        log.error("WITS fetch error: %s", e)
        return []


def _translate_ntm_ar(measures: list) -> list:
    """ترجمة ملخصات NTM إلى العربية دفعة واحدة عبر Claude."""
    if not CLAUDE_KEY:
        return measures
    to_translate = [m for m in measures if not m.get("measure_summary_ar") and m.get("measure_summary_en")][:10]
    if not to_translate:
        return measures
    texts    = [m["measure_summary_en"] for m in to_translate]
    numbered = "\n".join([str(i + 1) + ". " + t for i, t in enumerate(texts)])
    prompt   = "ترجم هذه الأوصاف القانونية للتدابير التجارية إلى العربية الفصحى. اكتب الرقم ثم الترجمة فقط:\n" + numbered
    try:
        r = requests.post(
            "https://api.anthropic.com/v1/messages",
            headers={"x-api-key": CLAUDE_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json"},
        ...
        if r.status_code == 200:
            lines = [l.strip() for l in r.json()["content"][0]["text"].strip().split("\n") if l.strip()]
            trs   = [re.sub(r"^[0-9]+[.)]\s*", "", l).strip() for l in lines]
            for i, m in enumerate(to_translate):
                if i < len(trs):
                    m["measure_summary_ar"] = trs[i]
    except Exception as e:
        log.error("NTM translate error: %s", e)
    return measures


@app.route("/api/v1/ntm/", methods=["GET"])
def ntm_list():
    """قائمة التدابير غير الجمركية مع فلاتر."""
    reporter  = request.args.get("reporter_iso", "").upper()
    partner   = request.args.get("partner_iso", "").upper()
    hs        = request.args.get("hs_code", "")
    year      = request.args.get("year", "")
    ntm_type  = request.args.get("ntm_type", "").upper()
    risk      = request.args.get("risk_level", "").lower()
    saudi     = request.args.get("impact_saudi", "")
    pg        = max(1, int(request.args.get("page", 1)))
    pg_size   = min(200, int(request.args.get("page_size", 50)))

    # تجميع كل البيانات من الكاش
    all_data = []
    with _ntm_lock:
        if reporter:
            all_data = list(_ntm_cache.get(reporter, []))
        else:
            for v in _ntm_cache.values():
                all_data.extend(v)

    # فلترة
    if partner:
        all_data = [m for m in all_data if m.get("partner_iso", "").upper() == partner]
    if hs:
        all_data = [m for m in all_data if (m.get("hs_code") or "").startswith(hs)]
    if year:
        all_data = [m for m in all_data if str(m.get("year", "")) == year]
    if ntm_type:
        all_data = [m for m in all_data if m.get("ntm_type", "").upper() == ntm_type]
    if risk:
        all_data = [m for m in all_data if m.get("risk_level", "") == risk]
    if saudi == "1":
        all_data = [m for m in all_data if m.get("impact_saudi_exports")]

    total     = len(all_data)
    page_data = all_data[(pg - 1) * pg_size: pg * pg_size]
    return jsonify({
        "ntm": page_data,
        "total": total,
        "page": pg,
        "page_size": pg_size,
        "pages": max(1, (total + pg_size - 1) // pg_size),
        "stats": {
            "total":   total,
            "high":    sum(1 for m in all_data if m.get("risk_level") == "high"),
            "medium":  sum(1 for m in all_data if m.get("risk_level") == "medium"),
            "low":     sum(1 for m in all_data if m.get("risk_level") == "low"),
            "saudi":   sum(1 for m in all_data if m.get("impact_saudi_exports")),
            "dispute": sum(1 for m in all_data if m.get("recommended_action") == "dispute"),
        }
    })


@app.route("/api/v1/ntm/fetch", methods=["GET", "POST"])
def ntm_fetch():
    """جلب التدابير من WITS API وتخزينها في الكاش."""
    reporter  = request.args.get("reporter_iso", "SAU").upper()
    partner   = request.args.get("partner_iso", "all")
    hs        = request.args.get("hs_code", "all")
    year      = request.args.get("year", "all")
    ntm_type  = request.args.get("ntm_type", "all")
    run_ai    = request.args.get("run_ai", "false").lower() == "true"
    translate = request.args.get("translate", "true").lower() == "true"

    started = datetime.now().isoformat()
    log_entry = {"started": started, "reporter": reporter, "status": "running", "count": 0}
    _ntm_log.append(log_entry)

    def _do():
        records = _fetch_wits_ntm(reporter, partner, hs, year, ntm_type)
        if translate and records:
            records = _translate_ntm_ar(records)
        if run_ai and CLAUDE_KEY and records:
            records = _batch_analyze_ntm(records[:5])  # حد أقصى 5 للتحليل الفوري
        with _ntm_lock:
            _ntm_cache[reporter] = records
        log_entry["status"]  = "success"
        log_entry["count"]   = len(records)
        log_entry["finished"] = datetime.now().isoformat()
        log.info("NTM fetch done: %s → %d records", reporter, len(records))
    ...
    return jsonify({
        "status":   "queued",
        "reporter": reporter,
        "message":  "جارٍ الجلب في الخلفية — راجع /api/v1/ntm/log للحالة",
        "params":   {"reporter": reporter, "partner": partner, "hs": hs, "year": year, "ntm_type": ntm_type}
    })


@app.route("/api/v1/ntm/analyze", methods=["POST"])
def ntm_analyze():
    """تحليل قانوني لتدبير NTM واحد عبر Claude."""
    if not CLAUDE_KEY:
        return jsonify({"error": "No Claude key"})
    try:
        m = request.get_json() or {}
        ntm_type   = m.get("ntm_type", "")
        wto_agree  = m.get("wto_agreement", "")
        reporter   = m.get("reporter_country") or m.get("reporter_iso") or ""
        partner    = m.get("partner_country")  or m.get("partner_iso")  or ""
        hs_code    = m.get("hs_code", "")
        product    = m.get("product_description") or m.get("affected_products") or ""
        ntm_code   = m.get("ntm_code", "")
        year       = str(m.get("year", ""))
        measure_en = m.get("measure_summary_en") or m.get("ntm_description") or ""
        legal_ref  = m.get("legal_text_reference", "")

        type_map = {
            "SPS": "اتفاقية التدابير الصحية والنباتية (SPS) — المادة 5",
            "TBT": "اتفاقية العوائق التقنية (TBT) — المادة 2",
            "QR":  "GATT 1994 المادة XI — الإلغاء العام للقيود الكمية",
            "CPS": "اتفاقية مكافحة الإغراق (Anti-Dumping Agreement)",
            "SBS": "اتفاقية الدعم والتدابير التعويضية (SCM)",
        }
        wto_context = type_map.get(ntm_type, "GATT 1994 + " + (wto_agree or "الاتفاقية ذات الصلة"))

        prompt = "\n".join([
            "أنت مستشار قانوني متخصص في اتفاقيات منظمة التجارة العالمية (WTO) ومتخصص في التدابير غير الجمركية (NTMs).",
            "حلّل التدبير التالي بأسلوب قانوني رسمي بالعربية الفصحى:",
            "",
            "| الحقل | القيمة |",
            "|---|---|",
            "| الدولة المطبِّقة | " + reporter + " |",
            "| الدولة الشريكة | " + partner + " |",
            "| كود HS | " + hs_code + " |",
            "| المنتج | " + product + " |",
            "| رمز التدبير | " + ntm_code + " |",
            "| نوع التدبير | " + ntm_type + " — " + (m.get("ntm_description") or "") + " |",
            "| وصف الإجراء | " + measure_en + " |",
            "| المرجع القانوني | " + legal_ref + " |",
            "| السنة | " + year + " |",
            "",
            "=== الإطار القانوني ===",
            wto_context,
            "",
            "=== الملخص التنفيذي ===",
            "اكتب 4-5 جمل عن جوهر التدبير وأهميته التجارية وأثره على صادرات المملكة العربية السعودية",
            "",
            "=== التحليل القانوني ===",
            "1. الأساس القانوني في " + wto_context,
            "2. هل يتوافق مع الاستثناءات المشروعة في GATT Art. XX؟",
            "3. الأثر على صادرات المملكة (النفط / البتروكيماويات / الألومنيوم / التمور / SABIC)",
            "4. حقوق الطعن وآليات DSU المتاحة",
            "",
            "=== مبررات التوصية ===",
            "اذكر التوصية العملية الواحدة من: (متابعة / إخطار / استشارة / STC / نزاع) ومبرراتها القانونية",
            "",
        ...
        if r.status_code == 200:
            text = r.json()["content"][0]["text"].strip()
            # استخراج الأقسام
            exec_m  = re.search(r"الملخص التنفيذي\s*={0,3}\s*([\s\S]*?)(?=\n={2,}|$)", text)
            legal_m = re.search(r"التحليل القانوني\s*={0,3}\s*([\s\S]*?)(?=\n={2,}|$)", text)
            action_m= re.search(r"مبررات التوصية\s*={0,3}\s*([\s\S]*)", text)
            return jsonify({
                "analysis":         text,
                "executive_summary": exec_m.group(1).strip()  if exec_m  else text[:600],
                "legal_analysis":    legal_m.group(1).strip() if legal_m else "",
                "action_rationale":  action_m.group(1).strip()if action_m else text[-400:],
            })
        return jsonify({"error": r.text[:300], "analysis": ""})
    except Exception as e:
        return jsonify({"error": str(e), "analysis": ""})


@app.route("/api/v1/ntm/analyze-batch", methods=["POST"])
def ntm_analyze_batch():
    """تحليل دُفعة من التدابير (حد أقصى 5) وإعادة النتائج."""
    if not CLAUDE_KEY:
        return jsonify({"error": "No Claude key", "results": []})
    try:
        body     = request.get_json() or {}
        measures = body.get("measures", [])[:5]
        results  = _batch_analyze_ntm(measures)
        return jsonify({"results": results, "count": len(results)})
    except Exception as e:
        return jsonify({"error": str(e), "results": []})


def _batch_analyze_ntm(measures: list) -> list:
    """تحليل دُفعة داخلي — يُعدَّل السجل مباشرة."""
    if not CLAUDE_KEY or not measures:
        return measures
    # نبني prompt واحد لكل التدابير
    items = []
    for i, m in enumerate(measures):
        items.append(
            str(i + 1) + ". " + (m.get("reporter_iso") or "") +
            " | " + (m.get("ntm_code") or "") +
            " | " + (m.get("ntm_type") or "") +
            " | HS:" + (m.get("hs_code") or "") +
            " | " + (m.get("measure_summary_en") or m.get("ntm_description") or "")[:120]
        )
    prompt = (
        "أنت محلل قانوني WTO. لكل تدبير من التدابير التالية اكتب سطراً واحداً بالعربية يتضمن:\n"
        "الرقم. [ملخص 15 كلمة] | [مستوى الخطر: منخفض/متوسط/مرتفع] | [التوصية: متابعة/إخطار/استشارة/STC/نزاع]\n\n"
        + "\n".join(items)
    )
    try:
        r = requests.post(
            "https://api.anthropic.com/v1/messages",
            headers={"x-api-key": CLAUDE_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json"},
            json={"model": "claude-opus-4-7", "max_tokens": 1000,
                  "messages": [{"role": "user", "content": prompt}]},
            timeout=30
        )
        if r.status_code == 200:
            ...
            for i, m in enumerate(measures):
                if i < len(lines):
                    parts = lines[i].split("|")
                    if len(parts) >= 1:
                        m["measure_summary_ar"] = re.sub(r"^[0-9]+[.)]\s*", "", parts[0]).strip()
    except Exception as e:
        log.error("NTM batch analyze error: %s", e)
    return measures


@app.route("/api/v1/ntm/stats", methods=["GET"])
def ntm_stats():
    """إحصائيات التدابير المخزّنة في الكاش."""
    all_data = []
    with _ntm_lock:
        for v in _ntm_cache.values():
            all_data.extend(v)
    ...
    for m in all_data:
        t = m.get("ntm_type", "OTH")
        by_type[t]    = by_type.get(t, 0) + 1
        c = m.get("reporter_iso", "")
        by_country[c] = by_country.get(c, 0) + 1
        r = m.get("risk_level", "low")
        by_risk[r]    = by_risk.get(r, 0) + 1
    return jsonify({
        "total":           len(all_data),
        "saudi_impacted":  sum(1 for m in all_data if m.get("impact_saudi_exports")),
        "dispute_cases":   sum(1 for m in all_data if m.get("recommended_action") == "dispute"),
        "by_type":         sorted(by_type.items(),    key=lambda x: -x[1])[:10],
        "by_country":      sorted(by_country.items(), key=lambda x: -x[1])[:10],
        "risk_distribution": by_risk,
        "cached_reporters":  list(_ntm_cache.keys()),
    })


@app.route("/api/v1/ntm/log", methods=["GET"])
def ntm_log():
    """سجل عمليات جلب NTM."""
    return jsonify({"log": _ntm_log[-20:], "total": len(_ntm_log)})


@app.route("/api/v1/ntm/clear", methods=["POST"])
def ntm_clear():
    """مسح كاش NTM."""
    with _ntm_lock:
        _ntm_cache.clear()
    return jsonify({"ok": True, "message": "NTM cache cleared"})


@app.route("/api/v1/dashboard/stats", methods=["GET"])
def dashboard_stats():
    """إحصائيات موحّدة للوحة القيادة (ePing + NTM)."""
    eping_data = _cache["data"]
    ntm_data   = []
    with _ntm_lock:
        for v in _ntm_cache.values():
            ntm_data.extend(v)
    return jsonify({
        "eping": {
            "total": len(eping_data),
            "sps":   sum(1 for n in eping_data if n["type"] == "SPS"),
            "tbt":   sum(1 for n in eping_data if n["type"] == "TBT"),
            "open":  sum(1 for n in eping_data if n["status"] == "مفتوح للتعليق"),
        },
        "ntm": {
            "total":   len(ntm_data),
            "high":    sum(1 for m in ntm_data if m.get("risk_level") == "high"),
            "saudi":   sum(1 for m in ntm_data if m.get("impact_saudi_exports")),
            "dispute": sum(1 for m in ntm_data if m.get("recommended_action") == "dispute"),
        },
        "last_eping_refresh": datetime.fromtimestamp(_cache["at"]).isoformat() if _cache["at"] else None,
    })
...
def health():
    """فحص حالة الخدمة."""
    ntm_total = sum(len(v) for v in _ntm_cache.values())
    return jsonify({
        "status":         "ok",
        "eping_cached":   len(_cache["data"]),
        "ntm_cached":     ntm_total,
